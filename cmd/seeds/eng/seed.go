...
func main() {
	...
	// create tokenizer
	enProseTokenizer := eng.NewEnProseTokenizer()
	entIndexer := entindexer.NewEntIndexer(db, t, enProseTokenizer, zlibInvertIndexCompresser)

	// load documents
	docs, err := data.LoadDocumentsFromTsv(data.MovieDocumentTsvPath)
	if err != nil {
